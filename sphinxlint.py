...
def checker(*suffixes, **kwds):
    ...
    def deco(func):
        ...
        checkers_by_name[func.name] = func
    ...
    class EnableDisableAction(argparse.Action):
        ...
        def __call__(self, parser, namespace, values, option_string=None):
            for value in values.split(","):
                if value == "all":
                    for checker in checkers_by_name.values():
                        checker.enabled = self.target
                else:
                    try:
                        checkers_by_name[value].enabled = True
                    except KeyError:
                        print(f"Unknown checker: {value}.")

    class EnableAction(EnableDisableAction):
        target = True

    class DisableAction(EnableDisableAction):
        target = False
...
def check_file(filename):
    ...
    try:
        ...
        return Counter({4: 1})
    return check_text(filename, text)


def main(argv=None):
    args = parse_args(argv)
    if args.list:
        for check in sorted(checkers_by_name.values(), key=lambda fct: fct.name):
            if check.enabled:
    ...
    if len(todo) < 8:
        results = map(check_file, todo)
    else:
        with multiprocessing.Pool() as pool:
            results = pool.map(check_file, todo)
            pool.close()
            pool.join()
